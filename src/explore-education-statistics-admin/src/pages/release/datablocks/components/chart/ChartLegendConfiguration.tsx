...
const ChartLegendConfiguration = ({
  ...
  const validationSchema = useMemo<ObjectSchema<FormValues>>(() => {
    let itemSchema: ObjectSchema<LegendItem> = Yup.object({
      dataSet: z.mixed<DataSet>().defined(), // ZOD equivalent of Yup needed here
      colour: z.string().required(
        params =>
      ...
      symbol: z.string<ChartSymbol>().optional(),
      lineStyle: z.string<LineStyle>().optional(),
      inlinePosition: z.string<LegendInlinePosition>().optional(),
    });

    if (capabilities.canPositionLegendInline) {
      itemSchema = itemSchema.shape({
        inlinePosition: z.string().oneOf<LegendInlinePosition>(
          ['above', 'below'],
          params =>
    ...
    if (capabilities.hasLineStyle) {
      itemSchema = itemSchema.shape({
        lineStyle: z.string().oneOf<LineStyle>(
          ['solid', 'dashed', 'dotted'],
          params =>
    ...
    if (capabilities.hasSymbols) {
      itemSchema = itemSchema.shape({
        symbol: z.string().oneOf<ChartSymbol>(
          [
            'circle',
            'cross',
            'diamond',
    ...
    let baseSchema: ObjectSchema<FormValues> = Yup.object({
      items: z.array<itemSchema>().defined().of(itemSchema),
      position: z.string<LegendPosition>().optional(),
    });
