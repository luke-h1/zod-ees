...
const ChartAxisConfiguration = ({
  ...
  const validationSchema = useMemo<ObjectSchema<FormValues>>(() => {
    let schema = z.object<FormValues>({
      size: z.number().positive('Size of axis must be positive'),

      tickConfig: z.string().oneOf<TickConfig>(
        ['default', 'startEnd', 'custom'],
        'Select a valid tick display type',
      ),
      tickSpacing: z.number().when('tickConfig', {
        is: 'custom',
        then: s =>
          s
    ...
    if (axisDefinition?.capabilities.canRotateLabel) {
      ...
      schema = schema.shape({
        label: z.object<Label>({
          text: z.string(),
          width: z.number().positive('Label width must be positive'),
        }),
      });
    }
