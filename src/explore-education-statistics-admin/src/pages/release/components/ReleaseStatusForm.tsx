  ...
  const validationSchema = useMemo(() => {
    const schema = z
      .object({
        ...
        internalReleaseNote: z.string().optional(),
        ...
        publishScheduled: z
          ...
          .refine(
            value => {
              return endOfDay(value as Date) >= endOfDay(new Date());
            },
            {
        ...
        updatePublishedDate: z.boolean().optional(),
      })
      .superRefine((data, ctx) => {
        if (data.approvalStatus === 'Approved') {
          ctx.addIssue({
            code: 'custom',
            message: 'Update published date is required',
            path: ['updatePublishedDate'],
          });

          ctx.addIssue({
            code: 'custom',
            message: 'Notify subscribers is required',
            path: ['notifySubscribers'],
          });
        }
      });
