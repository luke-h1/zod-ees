      ...
      validationSchema={toFormikValidationSchema(
        z
          .object({
            ...
            latestInternalReleaseNote: z.string().optional(),
            publishingStrategy: z.string().optional(),
            withReleaseId: z.string().optional(),
          })
          .superRefine((data, ctx) => {
            if (data.status === 'Approved') {
              ctx.addIssue({
                code: 'custom',
                path: ['publishingStrategy'],
                message: 'Choose when to publish',
              });

              ctx.addIssue({
                code: 'custom',
                path: ['latestInternalReleaseNote'],
                message: 'Enter an internal note',
              });
            }

            if (data.publishingStrategy === 'WithRelease') {
              ctx.addIssue({
                code: 'custom',
                path: ['withReleaseId'],
