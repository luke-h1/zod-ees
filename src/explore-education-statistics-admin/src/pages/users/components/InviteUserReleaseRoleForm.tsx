  ...
  return (
    <Formik<FormValues>
      ...
      validationSchema={z.object<FormValues>({
        releaseId: z
          .string({
            required_error: 'Choose release to give the user access to',
          })
          .refine(value => {
            return !userReleaseRoles.some(
              userReleaseRole => userReleaseRole.releaseId === value,
            );
          }),
        releaseRole: z.string({
          required_error: 'Choose release role for the user',
        }),
      })}
